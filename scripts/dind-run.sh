...
for optional in PI_VERSION PI_PROVIDER PI_THINKING_LEVEL OPIK_API_KEY OPIK_WORKSPACE OPIK_PROJECT_NAME MIN_TEST MIN_TEST_INCLUDE_TASK CLAUDE_TGZ_SOURCE CLAUDE_WHEEL_DIR_SOURCE HARBOR_CC_CLAUDE_TGZ_SOURCE HARBOR_CC_PY_WHEEL_DIR_SOURCE HARBOR_CC_WEB_MCP_ENABLED EXA_API_KEY; do
  if [[ -n "${!optional:-}" ]]; then
