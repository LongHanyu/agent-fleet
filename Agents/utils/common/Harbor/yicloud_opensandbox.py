    ...
    async def start(self, force_build: bool) -> None:
        ...
        created = await self._retry_control_plane_auth(
            ...
            sandbox.models.CreateSandboxReq(
                ...
                Image=sandbox.models.CreateSandboxReqImageInput(
                    **_image_request_fields(self._image_ref)
                ),
        ...
        try:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    delete_error = exc
                    self.logger.warning(
                        "YiCloud Sandbox cleanup request failed "
                        "sandbox_id=%s attempt=%s/3 error=%s",
                        sandbox_id,
                        attempt,
                        exc,
                    )
                    # A disconnected DELETE response can still mean the
                    # control plane accepted the request. Check state before
                    # retrying so cleanup does not create avoidable load.
                    try:
                        ...
                        if sandbox_id not in remaining_ids:
                            self.logger.info(
                                "YiCloud Sandbox cleanup confirmed after "
                                "request error sandbox_id=%s",
                                sandbox_id,
                            )
                            return
                    except Exception as status_exc:  # noqa: BLE001
                        self.logger.warning(
                            "YiCloud Sandbox cleanup status check failed "
                            "sandbox_id=%s error=%s",
                            sandbox_id,
                            status_exc,
                        )
            ...
            if delete_error is not None:
                self.logger.warning(
                    "YiCloud Sandbox cleanup deferred to lifecycle expiry "
                    "sandbox_id=%s error=%s",
                    sandbox_id,
                    delete_error,
                )
