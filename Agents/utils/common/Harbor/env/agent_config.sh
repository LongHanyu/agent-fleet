...
HARBOR_CC_WEB_MCP_ENABLED="${HARBOR_CC_WEB_MCP_ENABLED:-0}"
# Internal artifact path, never a second user-facing enable switch.
HARBOR_CC_WEB_MCP_SOURCE=""
if [[ "$HARBOR_CC_WEB_MCP_ENABLED" == "1" ]]; then
  HARBOR_CC_WEB_MCP_SOURCE="$(python3 "$REPO_ROOT/Agents/utils/common/mcp/build.py" "$LOCAL_WHEEL_DIR" --path-only)" || return 1
  EXA_API_KEY="${EXA_API_KEY:-anonymous}"
fi
...
if [[ "$AGENT" == "opencode" ]]; then
  OPENCODE_RUNTIME_SECRETS_JSON="$(
    OPENCODE_RUNTIME_SECRETS_JSON="$OPENCODE_RUNTIME_SECRETS_JSON" \
    HARBOR_CC_WEB_MCP_ENABLED="$HARBOR_CC_WEB_MCP_ENABLED" \
